...
const CreateAccount = (props) => {
  ...
  const { notify, simpleRedirect, encode, decode } = useContext(AuthContext);
  const [selectIndustry, setIndustry] = useState();
  const [formData, setFormdata] = useState();
  const [guestSignData, setGuestSigndata] = useState(false);
  const router = useRouter();
  const formId = process.env.NEXT_PUBLIC_CREATE_ACCOUNT_FORMID;
  ...
  const validationFormField = (e) => {
    ...
    if (targetObj.value === "") {
      ...
      inputFields.password.test(password)
  ...
  const [margeGuestOrderInAccountFn, { loading: margeOrderLoading }] =
    useMutation(mergeGuestOrderInAccount.loc.source.body);
  ...
  const [createCustomerAccountFn, { loading }] = useMutation(
    createCustomerAccount.loc.source.body,
  );
          ...
          >
            <Row>
                ...
                <Form.Row>
                  ...
                  <Form.Group as={Col} lg={12}>
                    ...
                    <Form.Control
                      ...
                      defaultValue={
                        (invitationEmail
                          ? invitationEmail.emailByInvitation.email
                          : "") ||
                  ...
                  <Form.Group as={Col} lg={12}>
                    ...
                    <Form.Text className="text-muted">
                      Password should be minimum 8 characters. It must contain
                      at least 1 of each of the following: Lower Case, Upper
              ...
              <Col sm={12} className="text-right text-md-left">
                <p className="text-danger pt-2 m-0 pb-4 d-none d-md-block">
                  * Required Fields
                </p>
                <Button
                  variant="secondary"
