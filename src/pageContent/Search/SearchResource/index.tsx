...
const SearchResourceData = dynamic(
  () => import("@Components/Search/SearchResourceData"),
);
...
const SearchResource = () => {
  ...
  const getSearchResult = (keyword) => {
    ...
    const filter = router.query.filter
      ? `pagetype:resource ${readySelectedSearchParam()}`
      : "pagetype:resource";
  ...
  return (
    <>
      ...
      {showLoader ? (
        <SearchSkeletonLoader pageName="resource" />
      ) : (
        <Container
                          ...
                          {[
                            isListView ? (
                              searchList.map((eachData, index) => (
                                <Row key={`newsRowNo_${loopCount + index}`}>
                                  <SearchResourceData
                                    resources={eachData}
                              ...
                              <Row key="newsRowNo_1" className="px-2">
