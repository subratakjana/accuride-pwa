import HomeOwners from "@PageContent/Markets/HomeOwners";
import gqlFetch from "@Graphql/Utilities/gqlFetch";
import { homeownersesContent } from "@Graphql/queriesgraphcms/homeowners.graphql";
...
const HomeOwnersPage = ({ homeownersess }) => (
  <HomeOwners homeownersess={homeownersess} />
);

export const getStaticProps = async ({ params }) => {
  let notFound = false;
  const DataLlist = await gqlFetch(homeownersesContent, { last: 0 }, "CMS");
  const getResponse = DataLlist.data.homeownerses;
  ...
  return {
    props: { homeownersess: getResponse, seodata: getSeoResponse },
    notFound,
    revalidate: 1800,
  };
};
